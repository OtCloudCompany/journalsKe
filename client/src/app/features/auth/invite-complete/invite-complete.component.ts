...
export class InviteCompleteComponent implements OnInit {
    ...
    submit(): void {
        ...
        if (!token) {
            this.errorMessage.set('Missing invite token. Request a new invite from an admin.');
            return;
        }
        ...
        this.authState.completeInvite(token, password).subscribe({
            next: (response) => {
                this.submitting.set(false);
                this.successMessage.set(response.detail ?? 'Invite accepted. You can now sign in.');
                this.form.reset();
            },
            error: (error) => {
                this.submitting.set(false);
                const detail = error?.error?.detail ?? 'Unable to complete invite. Request a new link.';
